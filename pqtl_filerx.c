...
#define MAX_PACKETS_INDEX 0xAFFF
...
int main(int argc, char **argv)
{
        ...
        short int ordering_counter;
        int my_counter_sz = sizeof(short int);    
    ...
    /*   if (pcap_setfilter(handle1, &fp) == -1) {
       ...
       for(i = 0l;;) {
           ...
           else 
           {
             ...
             {
                ordering_counter = 0;
                ordering_counter = ((short int)recvbuf[header_offset][k]) << 8;
                ordering_counter += (short int)recvbuf[header_offset + 1][k];
                sorting_table[order_block].order_index = ntohs(ordering_counter);
             ...
             {  
               if(buf_length[sorting_table[n].frame_index] >= 0)
               {
                    ...
                    buf_length[sorting_table[n].frame_index] -= header_offset +  sizeof(unsigned long) + my_counter_sz;
                    if(buf_length[sorting_table[n].frame_index] > 0)
                    {
