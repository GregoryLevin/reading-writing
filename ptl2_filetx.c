...
int main(int argc, char **argv)
{
        ...
        if (pcap_lookupnet(dev1, &net, &mask, errbuf) == -1) {
         ...
         for(i = 0; eof == 0; i++)
         {
            ...
            *(my_order_ptr + 3) = (unsigned char)block_counter[0].counter_bytes[0];
            *(my_order_ptr + 2) = (unsigned char)block_counter[0].counter_bytes[1];
            *(my_order_ptr + 1) = (unsigned char)block_counter[0].counter_bytes[2];
            *my_order_ptr = (unsigned char)block_counter[0].counter_bytes[3];

            block_counter[1].counter = (long int)htonl(file_block_counter%MAX_LONG_LIMIT);
            *(my_order_ptr + 7) = (unsigned char)block_counter[1].counter_bytes[0];
            *(my_order_ptr + 6) = (unsigned char)block_counter[1].counter_bytes[1];
            *(my_order_ptr + 5) = (unsigned char)block_counter[1].counter_bytes[2];
            *(my_order_ptr + 4)= (unsigned char)block_counter[1].counter_bytes[3];
            file_block_counter++;
