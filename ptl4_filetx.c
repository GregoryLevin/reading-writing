...
int main(int argc, char **argv)
{
        ...
        if( argc < 5) {
          ...
          printf("ptl4_filetx -tx input_interface -f filename_with_path\n");
          printf(" [-d] [delay_us] default 100000 us\n");
        ...
        if(argc > 1)
        {
          while(all_param_done == 0)
          {
             ...
             {
                delay_us = (long long int )strtoll(argv[param_index],0,10);
                if(delay_us == 0ll)
                  no_delay = 1;
                if(delay_us > 60ll)
                   delay_us-=60ll;
        ...
        printf("Delay us between buffers %lld\n", delay_us+60);//Jitter compensation
        ...
        if((int)(delay_us/1000000ll) > 0)
        {
          sec_sleep = (int)(delay_us/1000000ll);
          enable_sec_sleep = 1;
        }
        tv.tv_sec = (time_t)(0);
        tv.tv_nsec = (long)(delay_us*1000ll);
