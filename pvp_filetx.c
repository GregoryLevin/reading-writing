...
int main(int argc, char **argv)
{
        ...
        if( argc < 3) {
          ...
          printf("pvp_filetx interface pattern(hex)\n");
        ...
        pattern = (unsigned char)strtol(argv[2], 0, 16);
        ...
        memset(sendbuf + ether_header_sz, pattern,BUFFER_SIZE - ether_header_sz);
	    ...
	    crc = crc32(crc, (const unsigned char *)sendbuf, buf_size - sizeof(unsigned long));
	    memcpy(sendbuf + buf_size - sizeof(unsigned long), &crc, sizeof(unsigned long));
