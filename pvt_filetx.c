...
int main(int argc, char **argv)
{
        ...
        int my_ip_hdr_sz = sizeof(unsigned char);
        int predef_mac;
        unsigned long crc;
               ...
               else 
               {
                  ...
                  else 
                  {
                     if(strcmp(next_param_input,"-e") == 0)
                       ether_type = (unsigned int)strtol(argv[param_index],0,16);                    else
                     {
        ...
        if((block_dim.b_start.flag * block_dim.b_stop.flag * block_dim.b_increm.flag) == 0)
           printf("Size of the buffer      %d\n", buf_size);
        else
           printf("Buffer size start %d, finish %d with increment %d\n",
                 block_dim.b_start.data,block_dim.b_stop.data,block_dim.b_increm.data);
        if((block_dim.b_start.flag * block_dim.b_stop.flag * block_dim.b_increm.flag) == 0)
        {
        ...
        {
           if((block_dim.b_start.data > BUFFER_SIZE)||(block_dim.b_stop.data > BUFFER_SIZE))
           {
              printf("Sending buffer too big(%d MAX)\n", BUFFER_SIZE);
              return -2;
           }
        ...
        if (pcap_lookupnet(dev1, &net, &mask, errbuf) == -1) {
         ...
         for(i = 0; eof == 0; i++)
         {
            ...
            count = fread((&sendbuf[0] + ether_header_sz + 1), 1, buf_size - ether_header_sz - my_ip_hdr_sz - sizeof(long int), input_file);
            if(count <= 0)
            {
               eof = 1;
            ...
            *my_order_ptr = (unsigned char)(file_block_counter++);
