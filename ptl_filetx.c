...
int main(int argc, char **argv)
{
        ...
        int my_ip_hdr_sz = sizeof(short int);
        int predef_mac;
        unsigned long crc;
        ...
        if (pcap_lookupnet(dev1, &net, &mask, errbuf) == -1) {
         ...
         /*
         if(pcap_set_buffer_size(handle1, BUFFER_SIZE) != 0)
         {
             fprintf(stderr, "Can't set maximum buffer %d for device %s\n",
                     BUFFER_SIZE,dev1);
             return -10;
         }
         */
         ...
         for(i = 0; eof == 0; i++)
         {
            ...
            block_counter.counter = (short int)htons((short int)(file_block_counter++));
            *(my_order_ptr + 1) = (unsigned char)block_counter.counter_bytes[0];
            *my_order_ptr = (unsigned char)block_counter.counter_bytes[1];
