...
int main(int argc, char **argv)
{
        ...
        int num_set = 0;
        int padding = 0;
        long int mem_array_sz = LONG_MAX/(2*sizeof(temp_buffer));
        temp_buffer *tbuf_ptr;
        temp_buffer *nbuf_ptr;

        tbuf_ptr = (temp_buffer *)malloc(mem_array_sz);
        if(tbuf_ptr == 0)
        {
            printf("Cannot allocate memory pool\n");
            return -1;
        }
    ...
    /*   if (pcap_setfilter(handle1, &fp) == -1) {
       ...
       for(n = 0l; n < mem_array_sz; n++ )
       {
          nbuf_ptr->buf_length = 0;
          nbuf_ptr++;
       }
         ...
         while(just_receiving == 1) {
           ...
           if(header.len > 0)
           {
              ...
              for(n = 0; n < num_bytes;n++)
                 nbuf_ptr->tempbuf[n] = *(packet + n);
              //for(n = num_bytes; n < BUFFER_SIZE;n++)
              //   nbuf_ptr->tempbuf[n] = '0';
              nbuf_ptr->buf_length = num_bytes;
              nbuf_ptr++;
        ...
        while((one_time_processing == 1)&&(just_processing == 1))
        {
          ...
          while(all_bufs_processed == 0)
          {
            ...
            {             
              ...
              ordering_counter = ((long int)nbuf_ptr->tempbuf[header_offset + 3]) << 24;
              ordering_counter += ((long int)nbuf_ptr->tempbuf[header_offset + 2]) << 16;
              ordering_counter += ((long int)nbuf_ptr->tempbuf[header_offset + 1]) << 8;
              ordering_counter += (long int)nbuf_ptr->tempbuf[header_offset + 0];
              ...
              ordering_counter = ((long int)nbuf_ptr->tempbuf[header_offset + 7]) << 24;
              ordering_counter += ((long int)nbuf_ptr->tempbuf[header_offset + 6]) << 16;
              ordering_counter += ((long int)nbuf_ptr->tempbuf[header_offset + 5]) << 8;
              ordering_counter += (long int)nbuf_ptr->tempbuf[header_offset + 4];
                ...
                (temp_ordering >= current_buf)) 
              {
                 ...
                 else
                 {
                    ...
                    output_table[table_offset].buf_length = nbuf_ptr->buf_length;
                    memcpy((void *)&output_table[table_offset].buffer[0],(void *)&nbuf_ptr->tempbuf[0],nbuf_ptr->buf_length);
                 }
                 circle_buf++;
             }
              ...
              if((circle_buf > MAX_PACKETS_CATCH - 1)||(current_buf_last == 1))
                ...
                nbuf_ptr ++; //read one buffer more
                if((long int)(nbuf_ptr - tbuf_ptr) >= (mem_array_sz*sizeof(temp_buffer)))
                {
                  printf("Maximum size of the file exceeded\n");
                  return -7;
                }
              }
